...
class Client():
    ...
    def SplitBase(self, base):
      training_base = base.iloc[0:base.shape[0]-300,:]
      teste = base.iloc[base.shape[0]-300:base.shape[0],:]
    ...
    def RefreshModel(self, parInitial=False): #Constroi na primeira vez e atualiza, a partir da avaliação do servidor cetral
      ...
      regressor.fit(previsores, real_congestion, epochs = self.exp_epoch, batch_size = self.exp_batch_size)
    ...
    def GetPrevision(self, parSave=0,parLoadTextFromFile=False): #evalueta indica que é uma avaliação do modelo recebido como parametro, no caso do servidor

      test_vectors = self.LoadTestData(parLoadTextFromFile)
    ...
    def PlotResults(self):
      ...
      fig.savefig(self.exp_dir+"/client{id:0>3}_{num_plot:0>3}".format(id=self.id,num_plot=self.num_plot)+".png",dpi=300, bbox_inches='tight')
      fig.show()
      plt.close(fig)
...
exp_dir = "./Exp_000006/"
exp_dir=os.path.join(exp_dir, exp_time)
os.mkdir(exp_dir)
...
f.write("Batch Size:" + str(exp_batch_size)+"n")
...
client01_id=0
client01_training_path= "./Exp_000006/training_client01.csv"
client01_test_path = "./Exp_000006/test_client01.csv"
...
client02_id=1
client02_training_path='./Exp_000006/training_client02.csv'
client02_test_path =  './test_client02.csv'
...
for i in range(2):
  ...
  objClient1.GetPrevision(i,True)
  objClient1.PlotResults()
